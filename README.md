...
* [`src/clj/rhettro/`](src/clj/rhettro/): Backend and middleware source files (Clojure,
[Compojure](https://github.com/weavejester/compojure))
* [`src/cljs/rhettro/`](src/cljs/rhettro/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/rhettro/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/rhettro/`](test/cljs/rhettro/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `rhettro` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/rhettro/config.cljs) defaults to `true` in
...
```clj
(ns rhettro.example
  (:require [rhettro.config :as config])
...
```sh
port=2000 java -jar target/rhettro.jar
```
